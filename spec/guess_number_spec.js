...
describe('Guess Number', () => {
    ...
    it('should print Wrong Input，Input again when input one correct number', function () {
        let input = ['1 5 6 7'];
        guessNumber(input, printMock);
        expect(printMock).toHaveBeenCalledWith("1 correct");
    });

    it('should print Wrong Input，Input again when input contains two wrong position number', function () {
        let input = ['2 4 7 8'];
        guessNumber(input, printMock);
        expect(printMock).toHaveBeenCalledWith("2 and 4 wrong position");
    });

    it('should print Wrong Input，Input again when input contains two wrong position number and one correct number', function () {
        let input = ['0 3 2 4'];
        guessNumber(input, printMock);
        expect(printMock).toHaveBeenCalledWith("4 correct, 2 and 3 wrong position");
    });

    it('should print Wrong Input，Input again when input contains all wrong number', function () {
        let input = ['5 6 7 8'];
        guessNumber(input, printMock);
        expect(printMock).toHaveBeenCalledWith("all wrong");
    });

    it('should print Wrong Input，Input again when input contains all wrong poition number', function () {
        let input = ['4 3 2 1'];
        guessNumber(input, printMock);
        expect(printMock).toHaveBeenCalledWith("4 numbers position wrong");
    });

    it('should print Wrong Input，Input again when input contains all correct number', function () {
        let input = ['1 2 3 4'];
        guessNumber(input, printMock);
